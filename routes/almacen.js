...
router.get('/view', requirePerm('almacen_ver'), async (req, res, next) => {
  try {
    const { id, fuego = '', modelo = 0, medida = 0, orderby = 0 } = req.query;

    const almacen = await sql`SELECT * FROM almacen WHERE id = ${parseInt(id) || 0}`;
    ...
    const cubiertas = await sql(
      `SELECT c.*, mr.marca, mr.modelo AS modelo_nombre, m.medida
       ...
       ORDER BY ${orderExpr}`,
      [parseInt(id) || 0, fuego, '%' + fuego + '%', parseInt(modelo), parseInt(medida)]
    );

    res.render('almacen/view', {
      user: req.user, almacen: almacen[0], cubiertas, modelos, medidas, almacenes,
      currentPage: 'inicio',
      filtros: { fuego, modelo: parseInt(modelo), medida: parseInt(medida), orderby: parseInt(orderby) }
    });
  } catch (err) { next(err); }
});
